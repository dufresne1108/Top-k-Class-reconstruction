...
class Trainer(object):
    def __init__(self, start_num=0, end_num=10, net=None, save_path="",data_name="cifar100",epoch=50):
        ...
        self.main_net_path = save_path + "/fineTuning_" + str(start_num) + ".ptn"
    ...
    def test_acc(self, data_loader):
        ...
        for (inputs, labels) in data_loader:
            ...
            with torch.no_grad():
                ...
                prec1= self.accuracy(outputs.data[:, 0:self.end_num],
...
def model_test(model_num=5, save_path="", per_num=10):
    ...
    model_path = torch.load(save_path + "/fineTuning_" + str((model_num - 1) * 10) + ".ptn")
    net.load_state_dict(model_path)
    net.eval()
    feature_extract_list = []
    for j in range(model_num):
        feature_extract = resnet18(num_classes=100).cuda()
        model_path = torch.load(save_path + "/fineTuning_" + str(j * 10) + ".ptn")
    ...
    utils.save_acc_csv(save_file="/over_confident100-" + str(per_num) + ".csv", class_num=model_num * per_num,
                       acc=over_num.cpu().numpy(), model_name="fineTuning_100-" + str(per_num))
    utils.save_acc_csv(save_file="/over_confident100-" + str(per_num) + ".csv", class_num=model_num * per_num,
                       acc=over_num2.cpu().numpy(), model_name="fineTuning_Decoder100-" + str(per_num))

    utils.save_acc_csv(save_file="/decoder_acc100-" + str(per_num) + ".csv", class_num=model_num * per_num,
                       acc=correct_top1.cpu().numpy(), model_name="fineTuning_100-" + str(per_num))
    utils.save_acc_csv(save_file="/decoder_acc100-" + str(per_num) + ".csv", class_num=model_num * per_num,
                       acc=correct_combination, model_name="fineTuning_Decoder100-" + str(per_num))
